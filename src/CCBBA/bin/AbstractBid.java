...
public class AbstractBid{
    ...
    void calcBidForSubtask(Subtask j, AbstractSimulatedAgent agent){
        Vector<Subtask> oldBundle = agent.getLocalResults().getBundle();
        Vector<Subtask> oldPath = agent.getLocalResults().getPath();
        ...
        for (int i = 0; i < possiblePaths.size(); i++) { // Calculate utility for each new path
            ...
            if(newPathBid > maxPathBid){
                ...
                Vector<AbstractSimulatedAgent> localZ = agent.getLocalResults().getZ();
                Vector<Vector<AbstractSimulatedAgent>> pathOmega = new Vector<>();
    ...
    private PathUtility calcPathUtility(Vector<Subtask> path, AbstractSimulatedAgent agent){
    ...
    private double calcTimeOfArrival(Vector<Subtask> path, Subtask j, AbstractSimulatedAgent agent, PathUtility pathUtility){
        //Calculate time of arrival
        IterationLists localResults = agent.getLocalResults();
        double t_a;
        double t_corr;
    ...
    private Vector<Integer> getTimeConstraints(Subtask j, Vector<Subtask> path, AbstractSimulatedAgent agent){
        ...
        for(int i = 0; i < parentTask.getJ().size(); i++){
            ...
            if( isDependent && (hasWinner || (isInPath && isBehindInPath)) ){
                timeConstraints.add( agent.getLocalResults().getJ().indexOf(j) - parentTask.getJ().indexOf(j) + i );
            }
        }
    ...
    private double calcQuickestArrivalTime(Vector<Subtask> path, Subtask j, AbstractSimulatedAgent agent, PathUtility pathUtility){
        double delta_x;
        Dimension x_0;
    ...
    private PathUtility calcSubtaskUtility(Vector<Subtask> path, Subtask j, double t_a, AbstractSimulatedAgent agent, Vector<Vector<AbstractSimulatedAgent>> pathOmega){
        PathUtility pathUtility = new PathUtility();
    ...
    private double calcSubtaskScore(Vector<Subtask> path, Subtask j, double t_a, AbstractSimulatedAgent agent){
    ...
    private double calcUrgency(Subtask j, double t_a, AbstractSimulatedAgent agent){
...
//        return exp(- lambda * (t_a-t_start) );
    ...
    private double calcSigmoid(Vector<Subtask> path, Subtask j, AbstractSimulatedAgent agent){
        int i = path.indexOf(j);
        double delta_x;
        if(i == 0){
            Dimension x_i = agent.getLocation();
            Dimension x_f = j.getParentTask().getLocation();
...
//        return 1.0/( 1 + e);
    ...
    private double calcTravelCost(Vector<Subtask> path, Subtask j, AbstractSimulatedAgent agent){
        int i = path.indexOf(j);
        double delta_x;
        Dimension x_i;
        if(i == 0){
    ...
    private double calcMergePenalty(Vector<Subtask> path, Subtask j, AbstractSimulatedAgent agent, Vector<Vector<AbstractSimulatedAgent>> pathOmega){
        int i = path.indexOf(j);
        double C_split;
        double C_merge;
        ...
        Vector<Vector<AbstractSimulatedAgent>> overallOmega = localResults.getOverallOmega();
                ...
                if(overallOmega.get(i_last).size() > 0) { // is there a coalition at i - 1?
                    if(pathOmega.get(i).size() > 0) { // Is there a coalition at i?
                        ...
                        if(pathOmega.get(i).size() == overallOmega.get(i_last).size()){ // check size
                            for(AbstractSimulatedAgent coalMember : pathOmega.get(i)){ // check member by member
    ...
    private double calcSubtaskCost(Subtask j, Double g, Double p, AbstractSimulatedAgent agent){
        ...
        if( (cost_prop > 0.0)&&(cost_const <= 0.0) ){
            return (agent.getResources() - g - p ) * cost_prop;
        }
        else{
            return cost_const;
        }
    ...
    private double calcStartTime(double local_bid, Subtask j, Vector<Subtask> path, AbstractSimulatedAgent agent){
        double t_start = 0.0;
        return t_start;
    }

    private Dimension calcLocation(double local_bid, Subtask j, Vector<Subtask> path, AbstractSimulatedAgent agent){
