...
public class AbstractResultsCompiler extends AbstractAgent {
    private String directoryAddress;
    private int numAgents;
    protected Scenario environment;
    private Vector<IterationLists> receivedResults;

    public AbstractResultsCompiler(int numAgents, String directoryAddress){
    ...
    private void checkResults() throws IOException {
        ...
        if( (resultsAddress != null)&&(resultsAddress.size() == this.numAgents) ){
            ...
            Vector<IterationLists> receivedResults = new Vector<>();
    ...
    private void printResults( Vector<IterationLists> receivedResults ) throws IOException {
    ...
    private boolean compareResults( Vector<IterationLists> receivedResults ){
        ...
        for(IterationLists result : receivedResults){
            for(IterationLists comparedResult : receivedResults){
    ...
    private void printAllVectors( Vector<IterationLists> receivedResults ){
        ...
        // print values
        Vector<AbstractSimulatedAgent> agentList = new Vector<>();
        for(IterationLists result : receivedResults){
        ...
        for(int i = 0; i < receivedResults.get(0).getJ().size(); i++){
            ...
            for(IterationLists result : receivedResults){
                ...
                // obtain values
                Vector<AbstractSimulatedAgent> localZ = result.getZ();
    ...
    private void printAgentList(){
        ...
        //obtain values
        Vector<IterationLists> resultsToPrint = this.receivedResults;
        double x = 0.0;   double y = 0.0;   double z = 0.0;

        //print values
        for(int i = 0; i < resultsToPrint.size(); i++){
            IterationLists localResult = resultsToPrint.get(i);
            AbstractSimulatedAgent localAgent = localResult.getParentAgent();
            ...
            printWriter.printf("Resources:\t\t\t\t%d\n\n",localAgent.getLocalResults().getJ().size());
            ...
            for(int j = 0; j < localAgent.getLocalResults().getOverallBundle().size(); j++){
                Subtask bundleTask = localAgent.getLocalResults().getOverallBundle().get(j);
                ...
                if(j == (localAgent.getLocalResults().getOverallBundle().size() - 1) ) { printWriter.printf("}"); }
                else{ printWriter.printf("}, "); }
            }
            printWriter.printf("]\n");
            printWriter.printf("Path:\t\t\t\t\t[");
            for(int j = 0; j < localAgent.getLocalResults().getOverallPath().size(); j++){
                Subtask pathTask = localAgent.getLocalResults().getOverallPath().get(j);
                ...
                if(j == (localAgent.getLocalResults().getOverallPath().size() - 1) ) { printWriter.printf("}"); }
                else{ printWriter.printf("}, "); }
            }
            printWriter.printf("]\n\n");
    ...
    private void printReport(){
        ...
        Vector<AbstractSimulatedAgent> localZ = this.receivedResults.get(0).getZ();
        ...
        int winnerTemp;
        Vector<AbstractSimulatedAgent> agentList = getListOfAgents();
        ...
        for(int i = 0; i < resultsToPrint.size(); i++){
            ...
            for(int j = 0; j < localAgent.getLocalResults().getOverallBundle().size(); j++) {
                ...
                if (j < localAgent.getLocalResults().getOverallBundle().size() - 1){
                    if (i_j < 100) {
                        printWriter.printf("\t\t\t");
                    } else {
                        printWriter.printf("\t\t");
            ...
            printWriter.printf("Path:\t\t\t\t\t[");
            for(int j = 0; j < localAgent.getLocalResults().getOverallPath().size(); j++){
                Subtask bundleTask = localAgent.getLocalResults().getOverallPath().get(j);
                int i_j = localAgent.getLocalResults().getJ().indexOf(bundleTask);
                printWriter.printf("%d", i_j);
                if (j < localAgent.getLocalResults().getOverallPath().size() - 1){
                    if (i_j < 100) {
                        printWriter.printf("\t\t\t");
                    } else {
                        printWriter.printf("\t\t");
    ...
    private int countTasksDoneWithSensors(int numSensors){
        ...
        Vector<AbstractSimulatedAgent> localZ = receivedResults.get(0).getZ();
        for(Task v : V){
    ...
    private double calcScoreAchieved( Vector<IterationLists> receivedResults){
        double count = 0.0;
...
//        Vector<Double> localScore = receivedResults.get(0).getY();
    ...
    private double calcScoreAvailable( Vector<IterationLists> receivedResults){
        double count = 0;
        Vector<Task> V = environment.getTasks();
    ...
    private int countTasksDone( Vector<IterationLists> receivedResults){
        int count = 0;
        Vector<Task> V = environment.getTasks();
        Vector<AbstractSimulatedAgent> localZ = receivedResults.get(0).getZ();
        for(Task v : V){
    ...
    private int calcCoalitionsFormed(Vector<IterationLists> receivedResults){
        int count = 0;
        Vector<Task> V = environment.getTasks();
        Vector<AbstractSimulatedAgent> localZ = receivedResults.get(0).getZ();
        ...
        for(Task task : V){
            ...
            Vector<AbstractSimulatedAgent> taskWinners = new Vector<>();
            int i_j;
            AbstractSimulatedAgent localWinner;
    ...
    private double calcAvgCostPerResources( Vector<IterationLists> receivedResults){
        Vector<AbstractSimulatedAgent> agentList = new Vector<>();
        Vector<AbstractSimulatedAgent> localZ = receivedResults.get(0).getZ();
...
//            costPerAgent.setElementAt( costPerAgent.get(i)/ (double) bidsPerAgent.get(i), i);
    ...
    private Vector<AbstractSimulatedAgent> getListOfAgents(){
        Vector<IterationLists> resultsToPrint = this.receivedResults;
        Vector<AbstractSimulatedAgent> agentList = new Vector<>();
        for(int i = 0; i < resultsToPrint.size(); i++){
